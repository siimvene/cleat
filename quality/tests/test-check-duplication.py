...
try:
    ...
    # --- a formatter pass over both copies of a block: every line of the clone changes, the
    # clone is still the same two blocks the base had, and the density baseline holds it.
    repo2 = os.path.join(tmp, "repo2")
    joined = "".join("    total = compute_total(item.price, item.count, discount)  # step %d\n" % i for i in range(8))
    exploded = "".join("    total = compute_total(\n        item.price, item.count, discount\n    )  # step %d\n" % i for i in range(8))
    a2, b2, c2 = (os.path.join(repo2, "src", n) for n in ("a.py", "b.py", "c.py"))
    write(a2, "def total_a(items):\n    total = 0\n" + joined + "    return total\n")
    write(b2, "def total_b(items):\n    total = 0\n" + joined + "    return total\n")
    write(c2, "def other():\n    x = 1\n    y = 2\n    z = 3\n    return x + y + z\n")
    config2 = os.path.join(repo2, "quality.json")
    write(config2, json.dumps({"duplication": {"roots": ["src"], "languages": ["python"], "baseline": "duplication-baseline.json"}}))
    git(repo2, "init", "-q", "-b", "main")
    git(repo2, "config", "user.email", "t@example.com")
    git(repo2, "config", "user.name", "T")
    git(repo2, "add", "-A")
    git(repo2, "commit", "-q", "-m", "joined")
    write(a2, "def total_a(items):\n    total = 0\n" + exploded + "    return total\n")
    write(b2, "def total_b(items):\n    total = 0\n" + exploded + "    return total\n")
    code, out = run(config2, "--changed-only")
    check("a block the formatter exploded in both copies is a clone the base already held, not a new one",
          code == 0 and "the block has a twin" not in out and "1 clone pair(s) under changed lines already had every copy at the base" in out, out)
    write(c2, "def other():\n    x = 1\n    y = 2\n    z = 3\n    return x + y + z\n\n\ndef total_c(items):\n    total = 0\n" + exploded + "    return total\n")
    code, out = run(config2, "--changed-only")
