...
| Agent hooks: Stop runs the gates and blocks one stop; PreToolUse refuses `--write-baseline` and edits to `quality.json`, the baselines, the gates, the hooks (`--tighten`, which can only lower a baseline, is allowed) | the agent finishing with a red gate; the agent loosening policy mid-task | a session with different settings; a plain terminal |
...
Every baselined gate sorts each finding into one of five outcomes: **new** (fails), **worsened** — a recorded value went up (fails), **held**, **improved**, **stale** — the entry matched nothing. The last two mean the baseline is looser than the code; each is a NOTE with the command that tightens it (`--tighten`: the same rewrite, refused whenever it would accept something new or worse, so an agent may run it), and under `--strict` a failure, so CI keeps the file exact. A baseline records what measured it (tool, version, a hash of the gate's config); a run under a different one is noted the same way. Failure output names the fix and never the accept command.
