...
- Duplication: a clone under changed lines is new only when the base did not already hold every copy, compared with whitespace, brackets, commas and colons removed (the finder's own punctuation set). A formatter that joins, splits, re-indents or re-wraps lines changes every line of a block without writing a new one; before, the changed-lines judgment reported every such pre-existing clone as new (25 pairs on one pilot branch, 0 after, 24 held). A copy in a new file, moved between files, or rewritten beyond that still reads as new. Held pairs are one NOTE line. Whitespace itself still counts as a change: an indentation-only edit moves a line in or out of a block, so no scope ignores it.
- Changed lines: every call that takes the base ref passes `--end-of-options` first. The base is repo-chosen (`base_ref` in quality.json), and one beginning with `-` reached git as an option (`--output=` wrote a file from the hook). git output is decoded with replacement, so a base blob that is not UTF-8 is a text, not a traceback.
- Complexity `--only` honours the section's `exclude` globs. lizard applies `-x` to the files it walks and not to a file named on its command line, so the scoped run (the Stop hook's `--changed`) judged every changed test file the full run excludes: 80 "new production functions" under `tests/` on one pilot stop. Globs match the root-relative path as the walk spells it, never the absolute one.
