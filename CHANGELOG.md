...
- Duplication: a clone under changed lines is new only when the base did not already hold every copy, compared with whitespace, brackets and commas removed. A formatter that joins or splits lines changes every line of a block without writing a new one; before, the changed-lines judgment reported every such pre-existing clone as new (24 pairs on one pilot branch after `--ignore-all-space` alone, since that formatter joins lines rather than re-spacing them). A copy in a new file, moved between files, or rewritten beyond whitespace still reads as new. Held pairs are one NOTE line.
- Changed lines ignore whitespace (`git diff --ignore-all-space`): a formatter pass is not a change, so `gate.py --changed` and the duplication gate's changed-lines judgment no longer report every pre-existing clone under reformatted lines as new (25 pairs on one pilot branch, none introduced by it). A joined or split line still counts.
